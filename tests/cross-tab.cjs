...
(async()=>{
  ...
  try{
    ...
    const first=await context.newPage();await first.goto(base);await first.waitForFunction(()=>STORAGE_READY);
    await first.locator('#obGo').click();
    const second=await context.newPage();await second.goto(base);await second.waitForFunction(()=>STORAGE_READY);
    ...
    await second.locator('#conflictReload').click();await second.waitForFunction(()=>STORAGE_READY&&!STORAGE_CONFLICT);
    ...
    await third.waitForFunction(()=>!STORAGE_READY&&PROGRAMS.length>0);
    ...
    await third.locator('#conflictReload').click();await third.waitForFunction(()=>STORAGE_READY&&!STORAGE_CONFLICT);
